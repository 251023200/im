...
function searchOperation(){//查询
	...
	SysOperationModel.search.operationCode=$("#operationCode").val();
	initOperationTableList();
}
...
function chooseApp(){
	var searchApplicationUrl = app.server.auth + '/applications/search';
	$.addApplicationSelector.show(function(applications){
		console.log(applications);
		var application = applications[0];
		//originSelectedRoles = roles;
		//addApplicationCallback(roles);
		$('#upsert-app-id').val(application.id);
		$('#upsert-app-name').val(application.name);
		$('#upsert-app-code').val(application.code);
	},searchApplicationUrl);
}
	...
	$('#table').dataTable({
	   ...
	   "ajax" :function(data,callback,settings){
			...
			var param = {
				...
				"code" : SysOperationModel.search.operationCode,
