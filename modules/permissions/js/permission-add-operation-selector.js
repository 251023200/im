jQuery.addOperationSelector = (function($) {
	var _htmlUrl="/modules/permissions/html/permission-add-operation-selector.html";
	var _element = '#add-operation-selector';
	//var _operation_url = app.server.auth + '/operations?token='+getToken();
	var _operation_url = app.server.auth + '/operations';
	var _callback = null;
		...
		$.ajax({
			...
			success:function(data) {
				...
				_modal.find('#searchBtn').click(function(){//搜索
					searchObj.name = _modal.find('#operationName').val();
					searchObj.code = _modal.find('#operationCode').val();
					loadOperation();
				});
				_modal.find('#clearCondtion').click(function(){//重置
					var name = _modal.find('#operationName').val('');
					var code = _modal.find('#operationCode').val('');
					searchObj.name=undefined;
					searchObj.code=undefined;
	...
	var loadOperation = function(){
		$('#operation-selector-table').dataTable({
			destroy: true,
			"language": {
				"lengthMenu": "每页 _MENU_ 条记录",
				"zeroRecords": "没有可添加的操作",
