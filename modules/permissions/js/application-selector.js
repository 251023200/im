jQuery.addApplicationSelector = (function($) {
	var _htmlUrl="/modules/permissions/html/application-selector.html";
	var _element = '#application-selector';
	//var _operation_url = app.server.auth + '/applications?token='+getToken();
	var _operation_url = app.server.auth + '/applications';
	var _callback = null;
		...
		$.ajax({
			...
			success:function(data) {
				...
				_modal.find('#searchBtn').click(function(){//搜索
					searchObj.name = _modal.find('#applicationName').val();
					loadApplication();
				});
				_modal.find('#clearCondtion').click(function(){//重置
					var name = _modal.find('#applicationName').val('');
	...
	var loadApplication = function(){
		$('#application-selector-table').dataTable({
			destroy: true,
			"language": {
